/** Definition for NonSpitzerNonVar class
 * @file obs_nonspitzernonvar.cpp
 ...
 */
NonSpitzerNonVar::NonSpitzerNonVar() : Observations() {
 ...
 */
const std::vector<std::string>& NonSpitzerNonVar::getLcLibrary() {
	static bool initialized = false;
	static std::vector<std::string> fileList;
	
	if (!initialized) {
		FILE* hCatalog = fopen("nonspitzernonvar.cat", "r");
		if (hCatalog == NULL) {
			throw FileIo("Could not open file 'nonspitzernonvar.cat'.");
		}
		...
		while(!feof(hCatalog)) {
			...
			if (ferror(hCatalog)) {
				throw FileIo(std::string("Error while reading nonspitzernonvar.cat: ") + strerror(ferror(hCatalog)));
			}

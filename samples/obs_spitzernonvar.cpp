 ...
 */
SpitzerNonVar::SpitzerNonVar() : Observations() {
 ...
 */
const std::vector<std::string>& SpitzerNonVar::getLcLibrary() {
	static bool initialized = false;
	static std::vector<std::string> fileList;
	
	if (!initialized) {
		FILE* hCatalog = fopen("spitzernonvar.cat", "r");
		if (hCatalog == NULL) {
			throw FileIo("Could not open file 'spitzernonvar.cat'.");
		}
		...
		while(!feof(hCatalog)) {
			...
			if (ferror(hCatalog)) {
				throw FileIo(std::string("Error while reading spitzernonvar.cat: ") + strerror(ferror(hCatalog)));
			}

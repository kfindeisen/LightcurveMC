/** Definition for SpitzerVar class
 * @file obs_spitzervar.cpp
 ...
 */
SpitzerVar::SpitzerVar() : Observations() {
 ...
 */
const std::vector<std::string>& SpitzerVar::getLcLibrary() {
	static bool initialized = false;
	static std::vector<std::string> fileList;
	
	if (!initialized) {
		FILE* hCatalog = fopen("spitzervar.cat", "r");
		if (hCatalog == NULL) {
			throw FileIo("Could not open file 'spitzervar.cat'.");
		}
		...
		while(!feof(hCatalog)) {
			...
			if (ferror(hCatalog)) {
				throw FileIo(std::string("Error while reading spitzervar.cat: ") + strerror(ferror(hCatalog)));
			}
